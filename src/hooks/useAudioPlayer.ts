...
import { type AudioSession, updateSessionPosition } from "~/services/session";
...
export function useAudioPlayer({
  ...
  useEffect(() => {
    ...
    if (state.matches("ready")) {
      console.log(
        "▶️ [AudioPlayer] Audio ready, starting playback and transitioning to PLAYING",
      );
      audioEngine
        .play(audioUrl, {
          onEnded: () => {
            console.log(
              "🎵 [AudioPlayer] Audio ended, sending AUDIO_ENDED event",
            );
        ...
        .then((success) => {
          if (success) {
            ...
            console.log(
              "✅ [AudioPlayer] Audio started, sending PLAY event to enter PLAYING state",
            );
            send({ type: "PLAY" });
          }
        })
  ...
  useEffect(() => {
    ...
    if (
      ...
      savePositionTimeoutRef.current = setTimeout(() => {
        updateSessionPosition(currentSession.id, currentParagraph).then(
          (success) => {
            if (success) {
              console.log(
                `📍 Saved playback position: paragraph ${currentParagraph + 1}`,
              );
            }
          },
        );
  ...
  const handlePlayParagraph = useCallback(
    (index: number, forceReload = false) => {
      console.log(
        `🎯 [AudioPlayer] Playing paragraph ${index + 1}${forceReload ? " (force reload)" : ""}`,
      );
